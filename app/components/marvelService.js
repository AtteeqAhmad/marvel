...
let _sandbox = axios.create({
   baseURL: 'https://bcw-sandbox.herokuapp.com/api/Mark/heroes'
})

//variable controlls for Marvel 
...
export default class MarvelService {
   ...
   get ApiHeros() {
      return _state.apiHeross.map(h => new Hero(h))
   }

   get MyTeam() {
      return _state.myTeam.map(h => newHero(h))
   }
   ...
   addToTeam(id) {
      ...
      //// Send data to server
      //first parametter is appended on baseURL, second parameter is data to send 
   ...
   //get data 
   getMarvelData() {
      _marvelAPI.get('4{_characters}&offset=${_offset}&apikey=${_apiKey}')
         .then(res => {
